...
** 'floor(q) == trunc(q)' when 'q >= 0' or when 'q' is integer, ** otherwise 'floor(q) == trunc(q) - 1'.
*/
...
/*
** check whether cached closure in prototype 'p' may be reused, that is, ** whether there is a cached closure with the same upvalues needed by
...
#define checkGC(L,c) Protect( luaC_condGC(L,{L->top = (c);    /* limit of live values */ luaC_step(L); L->top = ci->top;})    /* restore top */ \
                     luai_threadyield(L); )
